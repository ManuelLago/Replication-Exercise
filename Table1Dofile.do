clear all
use "C:\Users\manue\OneDrive\Escritorio\Econometrics II UCD\Replication\Datasets\aer_pscore_data.dta", clear
...
pause on
capture log close
log using "outputtable1", replace text	

use "C:\Users\manue\OneDrive\Escritorio\Econometrics II UCD\Replication\Datasets\aer_data.dta", clear
...
merge 1:1 stfips cofips using "pscore1"
keep if _merge==3
drop _merge
...
preserve
	...
	ren v1 _other
	ren v2 _pre74
	merge 1:1 _varname using "outputtable1"
	drop _merge
	save "outputtable1", replace
restore
	...
	*resample pscores 1000 times and calculate a t-stat for EACH covariate on EACH rep
	...
	forval i = 1/1000{
		...
		*gen dfl weights
		sum chc
		local ED 					= r(mean)
		gen b_dflwgt				= (b_pscore/`ED')*((1-`ED')/(1-b_pscore))*(1-chc)			
...
merge 1:1 _varname using "outtputable1"
drop _merge
...
export excel using "outputtable1.xls", replace firstr(var)
log close